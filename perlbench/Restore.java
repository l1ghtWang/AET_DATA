...
{ 
  ...
  int len_data1 = Restore.readInput(br, data, data1, cut);
  int len_data2 = Restore.readInput(br2, data2, cut);
  int len_data3 = Restore.readInput(br3, data3, cut);
  int len_data4 = Restore.readInput(br4, data4, cut);
  int len_data5 = Restore.readInput(br5, data5, cut);
  int len_data6 = Restore.readInput(br6, data6, cut);
  int len_data7 = Restore.readInput(br7, data7, cut);
  int len_data8 = Restore.readInput(br8, data8, cut);
...
*/
   ...
   //System.out.println(length+"  "+(length/5)); 
	...
	int len_out_data2 = Restore.restoreData( data2, out_data2, data1, length, section);
	int len_out_data3 = Restore.restoreData( data3, out_data3, data1, length, section);
	int len_out_data4 = Restore.restoreData( data4, out_data4, data1, length, section);
	int len_out_data5 = Restore.restoreData( data5, out_data5, data1, length, section);
	int len_out_data6 = Restore.restoreData( data6, out_data6, data1, length, section);
	int len_out_data7 = Restore.restoreData( data7, out_data7, data1, length, section);
	int len_out_data8 = Restore.restoreData( data8, out_data8, data1, length, section);
...
public static int restoreData(ArrayList<Double> data2, ArrayList<Double> out_data2,ArrayList<Double> data1, int length, int section)
{
	...
	for(int k = 0; k < section; k++)
	{
		...
		int mean = len_segment/2*(1+2*k);
		Double diff = data2.get(mean) - data1.get(mean); 

		for(int j = len_segment*k; j < len_segment*(k+1); j++)
		{
