...
{ 
  ...
  while ((st = br8.readLine()) != null) 
  {
   ...
   for(int k = 0; k < section; k++)
   {
        ...
        Double diff1 = (Double) data2.get(seg_start) - data1.get(seg_start); 
        Double diff2 = (Double) data2.get(seg_end) - data1.get(seg_end);
        System.out.println("seg_start is "+ seg_start+" seg_end " + seg_end); 
        for(int j = seg_start; j < seg_end; j++)
        {
          
 
          Double diff = diff1 - (diff1-diff2)/len_segment*jj;
